...
MQTT_PORT = 1883
MQTT_TOPIC = "wisense/live"
MQTT_CLIENT_ID = "wisense-python-dashboard"
...
def connect_mqtt():
    """Connect to the local Mosquitto broker used by Node-RED."""
    try:
        ...
        client.loop_start()
        print(
            f"MQTT connected: {MQTT_BROKER}:{MQTT_PORT} "
            f"-> {MQTT_TOPIC}"
        )
        return client
        ...
        print("The CSI dashboard will continue, but Node-RED will receive no data.")
        return None
...
class WiSenseDashboard:
    def __init__(self, receiver, mqtt_client=None):
        ...
        self.figure.text(
            0.5,
            0.018,
            "CSV + MQTT: wisense/live  |  Press R to recalibrate  |  "
    ...
    def _write_log_row(self):
        """Save one reading to CSV and publish it to Node-RED through MQTT."""
        ...
        reading = {
            "timestamp": timestamp,
            "score": round(float(self.live_score), 3),
            "baseline": round(float(self.baseline), 3),
            "threshold": round(float(self.threshold), 3),
            "rssi": None if self.rssi is None else int(self.rssi),
            "status": status,
            "events": int(self.movement_events),
            "posture": "UNKNOWN",
        }
        ...
        if self.mqtt_client is not None:
            try:
                ...
                if result.rc != mqtt.MQTT_ERR_SUCCESS:
                    raise OSError(f"publish returned MQTT code {result.rc}")

                self.mqtt_error = None

            except (OSError, ValueError, TypeError) as error:
                if self.mqtt_error is None:
                    print(f"MQTT publishing error: {error}")
                self.mqtt_error = str(error)
...
def main():
    ...
    try:
        ...
        dashboard = WiSenseDashboard(receiver, mqtt_client)
        dashboard.run()
